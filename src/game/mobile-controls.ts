...
export function attachMobileControls(canvas:HTMLCanvasElement,callbacks:MobileControlCallbacks):MobileControlHandle{
 ...
 const down=(e:PointerEvent)=>{if(destroyed||e.pointerType!=='touch')return;e.preventDefault();pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});try{canvas.setPointerCapture(e.pointerId);}catch{}if(e.clientX<window.innerWidth*.42&&joystickPointer===null){joystickPointer=e.pointerId;joystickOrigin={x:e.clientX,y:e.clientY};return;}if(lookPointer===null)lookPointer=e.pointerId;};
 ...
 const release=(e:PointerEvent)=>{if(e.pointerType!=='touch')return;pointers.delete(e.pointerId);if(e.pointerId===joystickPointer){joystickPointer=null;joystickOrigin=null;(['forward','back','left','right'] as Action[]).forEach(a=>callbacks.setAction(a,false));}if(e.pointerId===lookPointer)lookPointer=null;try{if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);}catch{}};
